...
import { CustomContextMenu } from "./custom-context-menu.js";
...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    // Adicionar funcionalidade para os novos botões
    if (this.appConfig.toolbar.toggleCursorButton) {
      this.appConfig.toolbar.toggleCursorButton.addEventListener("click", evt => {
        const currentTool = this.pdfCursorTools?.activeTool || CursorTool.SELECT;
        const newTool = currentTool === CursorTool.SELECT ? CursorTool.HAND : CursorTool.SELECT;
        this.pdfCursorTools?.switchTool(newTool);
      });
    }

    if (this.appConfig.toolbar.rotatePageButton) {
      this.appConfig.toolbar.rotatePageButton.addEventListener("click", evt => {
        this.rotatePages(90); // Girar 90 graus no sentido horário
      });
    }
    
    // Inicializar o menu de contexto personalizado para "Copiar para Busca"
    if (appConfig.findBar && appConfig.mainContainer) {
      this.customContextMenu = new CustomContextMenu(
        appConfig.mainContainer,
        eventBus,
        {
          bar: appConfig.findBar.bar,
          findField: appConfig.findBar.findField
        }
      );
    }
